...
namespace RuntimeGizmos
{
    ...
    public class TransformGizmo : NetworkBehaviour
    {
        public TransformSpace space = TransformSpace.Global;
        //public TransformType transformType = TransformType.Move;
        public NetworkVariable<TransformType> transformType = new NetworkVariable<TransformType>(TransformType.Move);
        ...
        void OnDisable()
        {
            ClearTargetsServerRpc(); //Just so things gets cleaned up, such as removing any materials we placed on objects.
        ...
        void OnPostRender()
        {
            ...
            TransformType moveOrScaleType = (transformType.Value == TransformType.Scale || (isTransforming && translatingType == TransformType.Scale)) ? TransformType.Scale : TransformType.Move;
        ...
        {
            return transformType.Value == TransformType.Scale ? TransformSpace.Local : space;
        }

        public bool TransformTypeContains(TransformType type)
        {
            return TransformTypeContains(transformType.Value, type);
        }
        public bool TranslatingTypeContains(TransformType type, bool checkIsTransforming = true)
        {
            TransformType transType = !checkIsTransforming || isTransforming ? translatingType : transformType.Value;
        ...
        public float GetHandleLength(TransformType type, Axis axis = Axis.None, bool multiplyDistanceMultiplier = true)
        {
            ...
            if (transformType.Value == TransformType.All)
            {
        ...
        void SetSpaceAndType()
        {
            ...
            if (Input.GetKeyDown(SetMoveType)) transformType.Value = TransformType.Move;
            else if (Input.GetKeyDown(SetRotateType)) transformType.Value = TransformType.Rotate;
            else if (Input.GetKeyDown(SetScaleType)) transformType.Value = TransformType.Scale;
            //else if(Input.GetKeyDown(SetRectToolType)) type = TransformType.RectTool;
            else if (Input.GetKeyDown(SetAllTransformType)) transformType.Value = TransformType.All;

            if (!isTransforming) translatingType = transformType.Value;
            ...
            if (transformType.Value == TransformType.Scale)
            {
        ...
        IEnumerator TransformSelected(TransformType transType)
        {
            ...
            while (!Input.GetMouseButtonUp(0))
            {
                ...
                if (previousMousePosition != Vector3.zero && mousePosition != Vector3.zero)
                {
                    if (transType == TransformType.Move)
                    {
                        ...
                        translateTargetServerRpc(movement);
                        // for (int i = 0; i < targetRootsOrdered.Count; i++)
                        // {
                    ...
                    else if (transType == TransformType.Scale)
                    {
                        ...
                        scaleTargetServerRpc(targetScaleAmount, originalPivot);
                        // for (int i = 0; i < targetRootsOrdered.Count; i++)
                        // {
                        //     Transform target = targetRootsOrdered[i];
                    ...
                    else if (transType == TransformType.Rotate)
                    {
                        ...
                        rotateTargetServerRpc(rotationAxis, originalPivot, rotateAmount);
                        // for (int i = 0; i < targetRootsOrdered.Count; i++)
                        // {
                        //     Transform target = targetRootsOrdered[i];
            ...
            wrapUpTransformationServerRpc();
            // totalRotationAmount = Quaternion.identity;
            // totalScaleAmount = 0;
            // isTransforming = false;

            // SetTranslatingAxis(transformType.Value, Axis.None);

            // SetPivotPoint();
        }

        [ServerRpc]
        public void wrapUpTransformationServerRpc()
        {
            wrapUpTransformationClientRpc();
        }

        [ClientRpc]
        public void wrapUpTransformationClientRpc()
        {
            totalRotationAmount = Quaternion.identity;
            totalScaleAmount = 0;
            isTransforming = false;
        ...
        [ServerRpc]
        public void translateTargetServerRpc(Vector3 movement)
        {
            translateTargetClientRpc(movement);
        }

        [ClientRpc]
        public void translateTargetClientRpc(Vector3 movement)
        {
            for (int i = 0; i < targetRootsOrdered.Count; i++)
            {
                Transform target = targetRootsOrdered[i];
        ...
        [ServerRpc]
        public void scaleTargetServerRpc(Vector3 targetScaleAmount, Vector3 originalPivot)
        {
            scaleTargetClientRpc(targetScaleAmount, originalPivot);
        }

        [ClientRpc]
        public void scaleTargetClientRpc(Vector3 targetScaleAmount, Vector3 originalPivot)
        {
            for (int i = 0; i < targetRootsOrdered.Count; i++)
            {
                Transform target = targetRootsOrdered[i];
        ...
        [ServerRpc]
        public void rotateTargetServerRpc(Vector3 rotationAxis, Vector3 originalPivot, float rotateAmount)
        {
            rotateTargetClientRpc(rotationAxis, originalPivot, rotateAmount);
        }

        [ClientRpc]
        public void rotateTargetClientRpc(Vector3 rotationAxis, Vector3 originalPivot, float rotateAmount)
        {
            for (int i = 0; i < targetRootsOrdered.Count; i++)
            {
                Transform target = targetRootsOrdered[i];
        ...
        void GetTarget()
        {
            if (nearAxis == Axis.None && Input.GetMouseButtonDown(0))
            {
                ...
                if (Physics.Raycast(myCamera.ScreenPointToRay(Input.mousePosition), out hitInfo, Mathf.Infinity, selectionMask))
                {
                    ...
                    if (isAdding)
                    {
                        AddTargetServerRpc(GetTransformID(target));
                    }
                    else if (isRemoving)
                    {
                        RemoveTargetServerRpc(GetTransformID(target));
                    }
                    else if (!isAdding && !isRemoving)
                    {
                        ClearAndAddTargetServerRpc(GetTransformID(target));
                    }
                }
                else
                {
        ...
        [ServerRpc]
        public void AddTargetServerRpc(int targetID, bool addCommand = true)
        {
            AddTargetClientRpc(targetID, addCommand);
        }

        [ClientRpc]
        public void AddTargetClientRpc(int targetID, bool addCommand = true)
        {
        ...
        [ServerRpc]
        public void RemoveTargetServerRpc(int targetID, bool addCommand = true)
        {
            RemoveTargetClientRpc(targetID, addCommand);
        }

        [ClientRpc]
        public void RemoveTargetClientRpc(int targetID, bool addCommand = true)
        {
            Transform target = GetTransformFromID(targetID);
            if (target != null)
            {
        ...
        [ServerRpc]
        public void ClearTargetsServerRpc(bool addCommand = true)
        {
            ClearTargetsClientRpc(addCommand);
        }

        [ClientRpc]
        public void ClearTargetsClientRpc(bool addCommand = true)
        {
        ...
        [ServerRpc]
        void ClearAndAddTargetServerRpc(int targetID)
        {
            ClearAndAddTargetClientRpc(targetID);

            ClearTargetsClientRpc(false);
            AddTargetClientRpc(targetID, false);
        }

        [ClientRpc]
        public void ClearAndAddTargetClientRpc(int targetID)
        {
            Transform target = GetTransformFromID(targetID);
        ...
        void SetNearAxis()
        {
            ...
            if (nearAxis == Axis.None && (TransformTypeContains(TransformType.Move) || TransformTypeContains(TransformType.Scale)))
            {
                ...
                if (nearAxis == Axis.None)
                {
                    ...
                    TransformType transType = transformType.Value == TransformType.All ? TransformType.Move : transformType.Value;
        ...
        void SetHandlePlanes()
        {
            ...
            if (TranslatingTypeContains(TransformType.Move))
            {
                ...
                if (transformType.Value == TransformType.All) { planeSize *= allMoveHandleLengthMultiplier; }
